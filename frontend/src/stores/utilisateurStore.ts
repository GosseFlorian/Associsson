...
export const useUtilisateurStore = create<UtilisateurStore>((set) => ({
  ...
  fetchUtilisateur: async () => {
    ...
    try {
      ...
      set({
        errorUtilisateur:
          error instanceof Error ? error.message : 'Erreur inconnue', //instanceof = verifie si error bien creer a partir de Error
        chargementUtilisateur: false,
      });
    }
  },
  ...
  fetchUtilisateurById: async (id: number) => {
    ...
    try {
      const response = await apiFetch(`/utilisateur/${id}`);
