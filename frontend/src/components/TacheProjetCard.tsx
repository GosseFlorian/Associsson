...
export function TacheProjetCard({ idProjet }: TacheProjetCardProps) {
  ...
  const tachesProjet = taches.filter((tache) => tache.projet_id === idProjet);
  ...
  return (
    <>
      {tachesProjet.map((tache) => (
        <div className="tacheProjetCard-container" key={tache.id}>
          ...
          <div className="tache-info">
            <p className={`bg tacheCard ${tache.statut}`}>
              {formtext(tache.statut)}
            </p>
            <p className={`bg tacheCard ${tache.priorite}`}>
              {formtext(tache.priorite)}
            </p>
          </div>
        </div>
      ))}
