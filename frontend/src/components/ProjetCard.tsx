...
export function ProjetCard() {
  ...
  const fetchProjet = useProjetStore((state) => state.fetchProjet);
  ...
  const listeProjet = projets.filter(
    (projet) => projet.organisation_id === Number(idOrganisation)
  );
  ...
  return (
    <>
      {listeProjet.map((projet) => (
        <div className="projetCard-container" key={projet.id}>
          <h2 className="projetCard">{projet.titre}</h2>
          <p className="projetCard">{projet.description}</p>
          <p className="projetCard">
            Date de début : {formatDate(projet.date_debut)}
          </p>
          <p className="projetCard">Adresse : {projet.adresse}</p>
          <p
            className={
              projet.est_termine
                ? 'termine projetCard bg'
                : 'en_cours projetCard bg'
            }
          >
